...
import { PrintProposalButton, ProposalRestoreButton, ProposalStatusForm, ProposalVersionCompare } from "@/components/proposal-forms";
...
export default async function ProposalDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  const canManage=permissionPolicy.allows(session,PERMISSIONS.proposalManage);const canEdit=canManage&&!proposal.status.terminal;const latest=proposal.versions[0];
  ...
  return <><div className="page-head proposal-detail-head"><div><p className="eyebrow">{proposal.proposalNo} · {proposal.customer.name}</p><h1>{proposal.name}</h1><div className="proposal-meta"><span className="badge">{proposal.status.label}</span><span>v{proposal.version}</span><span>Owner: {proposal.owner.name}</span><span>Opportunity: <Link className="link" href={`/opportunities/${proposal.opportunity.id}`}>{proposal.opportunity.name}</Link></span></div></div><div className="proposal-head-actions"><PrintProposalButton/>{canEdit&&<Link className="secondary" href={`/proposals/${proposal.id}/edit`}><Pencil aria-hidden="true"/>แก้ไข</Link>}{canManage&&<Link className="primary" href={`/quotes/new?proposalId=${proposal.id}`}>Create Quotation</Link>}</div></div>
    <div className="proposal-workspace"><main><section className="card"><div className="card-header"><div><strong>Proposal Overview</strong><small>ข้อมูลจาก version {latest.versionNumber}</small></div></div><div className="card-body detail-grid"><div><p className="detail-label">Expire Date</p><p className="detail-value">{latest.expireDate?.toLocaleDateString("th-TH",{timeZone:"Asia/Bangkok"})??"—"}</p></div><div><p className="detail-label">Tags</p><p className="detail-value">{tags.join(", ")||"—"}</p></div><div className="field full"><p className="detail-label">Description</p><p>{latest.description||"—"}</p></div></div></section><div className="proposal-sections">{latest.sections.map((section)=><section className="card proposal-section" key={section.sectionCode}><div className="card-header"><div><span className="proposal-section-index">{String(section.sortOrder+1).padStart(2,"0")}</span><strong>{section.title}</strong></div><span className="badge muted">{section.sectionCode}</span></div><div className="card-body"><div className="proposal-content" style={{whiteSpace:"pre-wrap"}}>{section.content||"—"}</div></div></section>)}</div><ProposalVersionCompare versions={versionCompare}/></main><aside><ProposalStatusForm key={`status-${proposal.version}`} proposalId={proposal.id} version={proposal.version} transitions={canManage?transitions:[]}/><section className="card"><div className="card-header"><strong>Quotation</strong></div><div className="card-body">{proposal.quotes.map((quote)=><p key={quote.id}><Link className="link" href={`/quotes/${quote.id}`}>{quote.quoteNo}</Link> <span className="badge muted">{quote.status}</span></p>)}{!proposal.quotes.length&&<p className="help">ยังไม่มี Quotation ที่เชื่อมกับ Proposal นี้</p>}</div></section></aside></div>
