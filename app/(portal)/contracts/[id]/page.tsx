...
export default async function ContractDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const visibleTransitions = transitions.filter((edge) => approvalEnabled || (allowed.statusCode !== "PENDING_APPROVAL" && edge.toStatusCode !== "PENDING_APPROVAL")).filter((edge) => hasPermission(edge.requiredPermission ?? PERMISSIONS.contractManage) && (!edge.makerChecker || contract.ownerId !== session.id));
  ...
  return <><div className="page-head"><div><p className="eyebrow">{contract.contractNo} · v{contract.version}</p><h1>{contract.name}</h1><p>{contract.contractType.name} · <span className="badge">{contract.status.label}</span></p></div><Link className="secondary" href="/contracts">Back to portfolio</Link></div>
    ...
    <ContractWorkflowControls contractId={contract.id} version={contract.version} transitions={visibleTransitions.map((item) => ({ code: item.toStatusCode, label: statusMap.get(item.toStatusCode)?.label ?? item.toStatusCode })).sort((a, b) => (statusMap.get(a.code)?.sortOrder ?? 0) - (statusMap.get(b.code)?.sortOrder ?? 0))} canUploadDocument={canManage} canSign={canSign} canCreateServiceOrder={canCreateServiceOrder} documents={cleanDocuments} serviceOrders={contract.serviceOrders.map((order) => ({ id: order.id, orderNo: order.orderNo, status: order.status }))} />
