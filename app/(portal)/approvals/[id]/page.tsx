...
export default async function ApprovalDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [request, approvalEnabled] = await Promise.all([prisma.approvalRequest.findFirst({ where: { id: (await params).id, quoteVersion: { quote: { opportunity: buildOpportunityScopeWhere(context) } } }, include: { quoteVersion: { include: { quote: true } }, policyVersion: true, steps: { orderBy: [{ sequence: "asc" }, { stepCode: "asc" }] }, decisions: { orderBy: { decidedAt: "asc" } } } }), isApprovalWorkflowEnforced("QUOTE_APPROVAL")]);
  if (!request) notFound();
  const actionable = request.steps.find((step)=>["PENDING","ESCALATED","DELEGATED"].includes(step.status));
  return <><div className="page-head"><div><p className="eyebrow">{request.quoteVersion.quote.quoteNo} · version {request.quoteVersion.versionNumber}</p><h1>Approval {request.status}</h1></div></div><section className="card"><div className="card-body"><p>Policy version: {request.policyVersion.version} · hash {request.quoteVersionHash}</p><ol>{request.steps.map((step)=><li key={step.id}>{step.stepCode} — {step.status} ({step.requiredPermission})</li>)}</ol><h3>Decision evidence</h3><ul>{request.decisions.map((decision)=><li key={decision.id}>{decision.decision} by {decision.actorId} at {decision.decidedAt.toISOString()} — {decision.reason}</li>)}</ul></div></section>{!approvalEnabled&&<p className="notice">Approval ถูกพักไว้ จึงไม่สามารถ Approve, Reject, Return, Delegate หรือ Escalate ได้ในขณะนี้</p>}{approvalEnabled&&actionable&&<section style={{marginTop:20}}><ApprovalDecisionForm requestId={request.id} stepId={actionable.id} version={request.version}/></section>}</>;
}
