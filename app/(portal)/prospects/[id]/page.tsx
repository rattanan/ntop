...
export default async function ProspectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    <div className="customer-hero">
      ...
      <div className="actions record-head-actions">{prospect.convertedLead && <Link className="primary" href={`/leads/${prospect.convertedLead.id}`}>เปิด Lead {prospect.convertedLead.leadNumber}<ArrowRight aria-hidden="true" /></Link>}{canConvert && !prospect.convertedLead && <ProspectConvertAction id={id} version={prospect.version} hasContact={Boolean(primaryContact)} contactName={primaryContact?.name}/>} {permissions.has(PERMISSIONS.prospectSoftDelete)&&!prospect.convertedLead&&<ProspectSoftDeleteAction id={id} version={prospect.version}/>} {canUpdate && <Link className="secondary" href={`/prospects/${id}/edit`}><Pencil aria-hidden="true" />แก้ไข</Link>}</div>
    </div>
