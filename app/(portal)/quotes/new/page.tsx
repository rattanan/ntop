...
export default async function NewQuote({ searchParams }: { searchParams: Promise<{ proposalId?: string; quoteId?: string }> }) {
  ...
  return <><div className="page-head"><div><p className="eyebrow">Versioned Quotation</p><h1>{sourceQuote ? "แก้ไขรายละเอียด / สร้าง Quotation Version ใหม่" : "สร้างใบเสนอราคา"}</h1><p>{sourceQuote ? "ระบบจะเก็บ version เดิมเป็นหลักฐานและสร้าง version ใหม่จากรายละเอียดที่แก้ไข" : proposal ? "Quotation นี้จะเชื่อมกับ Proposal ที่เลือก" : "กรอกข้อมูล Header และเพิ่ม Product detail ได้หลายรายการ"}</p>{!approvalEnabled&&<p className="notice">Approval ถูกพักไว้ชั่วคราว ผู้มีสิทธิ์แก้ไขสามารถเลือก Status ของ version ใหม่ได้</p>}</div></div><GovernedQuoteForm quoteId={sourceQuote?.id} proposalId={sourceQuote?.proposalId ?? proposal?.id ?? undefined} initialOpportunityId={sourceQuote?.opportunityId ?? proposal?.opportunityId} initialValidUntil={sourceVersion?.validUntil?.toISOString().slice(0,10)} initialNotes={sourceVersion?.notes ?? undefined} initialStatus={sourceVersion?.status} statusOverrideEnabled={Boolean(sourceQuote)&&!approvalEnabled} initialLines={initialLines} products={products.map((item) => ({ id:item.id,code:item.code,name:item.name,listPrice:item.listPrice.toFixed(4),floorPrice:item.floorPrice?.toFixed(4)??null }))} opportunities={opportunities.map((item) => ({ id: item.id, name: item.name, customerName: item.customer.name }))}/></>;
}
