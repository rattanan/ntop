...
import { QuoteSubmitForm } from "@/components/workflow-forms";
...
export default async function QuoteDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!quote) notFound();
  return <><div className="page-head"><div><p className="eyebrow">{quote.quoteNo} · {quote.customer.name}</p><h1>Quote Versions</h1></div></div>{quote.versions.map((version) => <section className="card" key={version.id} style={{ marginBottom: 20 }}><div className="card-header">Version {version.versionNumber} · {version.status}</div><div className="card-body"><div className="detail-grid"><div><p className="detail-label">Total</p><p className="detail-value">{version.total.toFixed(2)} {version.currency}</p></div>{session.role !== "VIEWER"&&<><div><p className="detail-label">Cost</p><p className="detail-value">{version.totalCost.toFixed(2)}</p></div><div><p className="detail-label">Margin</p><p className="detail-value">{version.grossMarginPct.toFixed(4)}%</p></div></>}</div><ul>{version.items.map((item)=><li key={item.id}>{item.productCode} — {item.productName} × {item.quantity.toFixed(4)} = {item.lineTotal.toFixed(2)}</li>)}</ul>{version.status === "DRAFT" && session.role !== "VIEWER" && <QuoteSubmitForm quoteId={quote.id} quoteVersionId={version.id}/>} {version.approvalRequests.map((request)=><p key={request.id}><a className="link" href={`/approvals/${request.id}`}>Approval {request.status} →</a></p>)}</div></section>)}{!quote.versions.length&&<section className="card"><div className="card-body">Legacy quote นี้ยังอ่านได้ แต่ยังไม่มี governed version</div></section>}</>;
}
