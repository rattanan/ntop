...
export default async function LeadDetail({params}:{params:Promise<{id:string}>}){
  ...
  return <>
    ...
    <div style={{marginTop:20}}><LeadInsightPanel id={id} status={lead.enrichmentStatus} canUpdate={canCoreUpdate} initialDraft={draft} summary={lead.aiSummary} scores={{opportunity:lead.aiOpportunityScore,risk:lead.aiRiskScore,confidence:lead.aiConfidenceScore}} updatedAt={lead.enrichmentUpdatedAt?.toLocaleString("th-TH",{timeZone:"Asia/Bangkok"})??null}/></div>

    <section className="card" style={{marginTop:20}}><div className="card-header"><div><strong>เอกสาร Lead</strong><small>เอกสารจาก Prospect จะแสดงอัตโนมัติหลัง Convert โดยไม่สร้างไฟล์ซ้ำ</small></div></div><div className="card-body"><LeadDocumentPanel id={id} documents={lead.salesDocuments.map(item=>({id:item.id,fileName:item.fileName,category:item.category,mimeType:item.mimeType,formattedSize:bytes(item.sizeBytes)}))} canUpdate={canCoreUpdate}/></div></section>

    <section className="card" style={{marginTop:20}}><div className="card-header"><div><strong>กิจกรรมและประวัติ</strong><small>Timeline ตามเวลาที่บันทึกในฐานข้อมูล</small></div>{canAddActivity&&<LeadActivityDialog leadId={lead.id}/>}</div><div className="card-body timeline">
      {lead.activities.map(item=><article key={item.id}><p><strong>{item.subject}</strong> · {item.type} · {item.owner.name}{item.notes&&<> <br/>{item.notes}</>}<br/><small>{item.createdAt.toLocaleString("th-TH",{timeZone:"Asia/Bangkok"})}</small></p></article>)}
