...
import { OpportunityProbabilityForm, OpportunityTransitionForm } from "@/components/workflow-forms";
...
export default async function OpportunityDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <section className="related-information" aria-label="Opportunity related information">
      ...
      <section className="card" id="competitors"><div className="card-header"><div><strong>Competitors &amp; Win Strategy</strong><small>{opportunity.competitors.length} รายการ</small></div></div><div className="card-body related-list">{opportunity.competitors.map(item=><article key={item.id}><div><span className={`threat-dot ${item.threatLevel.toLowerCase()}`} aria-hidden="true"/><strong>{item.competitorName}</strong>{item.incumbentFlag&&<span className="badge muted">Incumbent</span>}</div><p>{item.winStrategy??item.differentiation??"ยังไม่มีกลยุทธ์"}</p><small>Threat {item.threatLevel}{item.estimatedPrice?` · ${new Intl.NumberFormat("th-TH",{style:"currency",currency:opportunity.currency,maximumFractionDigits:0}).format(Number(item.estimatedPrice))}`:""}</small></article>)}{!opportunity.competitors.length&&<div className="compact-empty">ยังไม่มีข้อมูลคู่แข่ง</div>}</div></section>
    </section>
