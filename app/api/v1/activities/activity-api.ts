...
import { ActivityAccessError, ActivityConflictError, ActivityIdempotencyConflictError, ActivityValidationError } from "@/lib/activity/activity-service";
...
export function activityApiError(error: unknown, correlationId: string) {
  ...
  else if (error instanceof ActivityIdempotencyConflictError) { status = 409; code = "IDEMPOTENCY_CONFLICT"; message = error.message; }
