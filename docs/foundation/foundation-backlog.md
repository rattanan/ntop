...
| FND-002 | ADR set for modular monolith, REST, MySQL8, search, queue, cache, object storage | Enterprise Architecture | FND-001 | ADR-001/002/004 accepted; remaining product-specific ADRs may remain proposed with evidence plans | Ready for DoR (partial ADR set) |
| FND-003 | Define module dependency map and forbidden cross-module writes | Application Architecture | ADR-001 accepted | Customer/Sales/Presales/Commercial/Order/Integration/Audit boundaries mapped and statically enforceable design documented | Ready for DoR |
...
| FND-010 | Provision representative MySQL 8 integration test environment | Platform/DBA | ADR-002 accepted, environment approval | cluster/client compatibility, TLS, least privilege and connectivity evidence | Ready for DoR |
| FND-011 | Establish forward-only expand/backfill/verify/contract migration workflow | Data Architecture/DBA | FND-010 | sample migration supports backward compatibility and rollback procedure; no production data | Ready for DoR |
...
| FND-019 | Publish `/api/v1` conventions and OpenAPI governance | Application Architecture | ADR-004 accepted, FND-003/006 | cursor/filter/error/idempotency/version rules lintable; contract review complete | Ready for DoR |
