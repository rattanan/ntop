...
export function LeadActivityForm({ leadId }: { leadId: string }) {
  ...
  const key = useState(() => crypto.randomUUID())[0];
  return <form action={action} className="card form-card"><div className="card-body"><input type="hidden" name="idempotencyKey" value={key}/><div className="form-section"><h2>บันทึกกิจกรรมและ Follow-up</h2><div className="form-grid"><FormField label="หัวข้อ" name="subject" required error={state.errors?.subject}><Input name="subject" required error={!!state.errors?.subject}/></FormField><FormField label="ประเภท" name="type" required><select className="control" name="type" defaultValue="CALL">{ACTIVITY_TYPES.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></FormField><FormField label="วันเวลากิจกรรม" name="activityAt" required error={state.errors?.activityAt}><Input name="activityAt" type="datetime-local" required error={!!state.errors?.activityAt}/></FormField><FormField label="ติดตามครั้งถัดไป" name="nextFollowUpAt"><Input name="nextFollowUpAt" type="datetime-local"/></FormField><div className="field full"><FormField label="รายละเอียด / Outcome / Next action" name="notes"><Textarea name="notes"/></FormField></div></div></div><FormNotice state={state}/><div className="actions"><button className="primary" disabled={pending}>{pending?"กำลังบันทึก…":"บันทึกกิจกรรม"}</button></div></div></form>;
}
