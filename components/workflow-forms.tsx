...
type OpportunityTransitionRequirement = { id: string; requirementNumber: string; title: string; mandatoryFlag: boolean; status: string; feasibilityStatus: string };
...
export function OpportunityTransitionForm({ opportunityId, version, stage, expectedCloseAt, transitions, requirements }: { opportunityId: string; version: number; stage: string; expectedCloseAt?: string; transitions: OpportunityTransitionOption[]; requirements: OpportunityTransitionRequirement[] }) {
  const [state, action, pending] = useActionState(transitionOpportunity.bind(null, opportunityId), initial);
  const key = useIdempotencyKey();
  const preferredTransition = transitions.find((transition)=>transition.command==="FORWARD")??transitions[0];
  const [selectedId,setSelectedId] = useState(preferredTransition?.id??"");
    ...
    <div className="card-body">
      ...
      <div className="form-section"><h2>Transition ขั้นตอนขาย</h2><p>สถานะปัจจุบัน: <strong>{opportunityStageLabel(stage)}</strong> · version {version}</p>
        {selected ? <div className="form-grid">
          ...
          {selected.requiredFields.includes("requirements")&&<section className="transition-requirement-evidence field full" aria-labelledby={`transition-requirements-${opportunityId}`}><div className="transition-evidence-head"><div><strong id={`transition-requirements-${opportunityId}`}>Requirement ที่ใช้เป็นหลักฐาน</strong><small>รายการที่บันทึกไว้ใน Opportunity และใช้ประกอบ Stage Gate</small></div><span className="badge muted">{requirements.length} รายการ</span></div>{requirements.length?<ul>{requirements.map((requirement)=><li key={requirement.id}><div><span className="badge">{requirement.requirementNumber}</span><strong>{requirement.title}</strong>{requirement.mandatoryFlag&&<span className="required-mark">Required</span>}</div><small>{requirement.status} · {requirement.feasibilityStatus}</small></li>)}</ul>:<p className="notice notice-warning" role="status">ยังไม่มี structured requirement กรุณาเพิ่ม Requirement ก่อนเดินหน้า Stage</p>}<Link className="link transition-evidence-link" href="#requirements">ดูรายละเอียด Requirements ทั้งหมด</Link></section>}
