...
export function OpportunityTransitionForm({ opportunityId, version, stage, expectedCloseAt, stages }: { opportunityId: string; version: number; stage: string; expectedCloseAt?: string; stages: ReadonlyArray<{value:string;label:string}> }) {
  const [state, action, pending] = useActionState(transitionOpportunity.bind(null, opportunityId), initial);
  const key = useIdempotencyKey();
  const [targetStage,setTargetStage] = useState(stages[0]?.value??"");
  return <form action={action} className="card form-card">
    <div className="card-body">
      ...
      <div className="form-section"><h2>Transition ขั้นตอนขาย</h2><p>สถานะปัจจุบัน: <strong>{opportunityStageLabel(stage)}</strong> · version {version}</p>
        {targetStage ? <div className="form-grid">
          <FormField label="ขั้นตอนปลายทาง" name="transitionRoute" required help="เลือกเปลี่ยนไปยังขั้นตอนใดก็ได้ โดยไม่จำกัดเส้นทาง"><select className="control" value={targetStage} onChange={(event)=>setTargetStage(event.target.value)} aria-label="ขั้นตอนปลายทาง">{stages.map((option)=><option key={option.value} value={option.value}>{option.label} ({option.value})</option>)}</select></FormField>
          ...
          <div className="field full"><p className="help">ระบบจะสร้างคำสั่ง Transition ตามขั้นตอนปลายทางอัตโนมัติ และบันทึกประวัติพร้อม Audit ทุกครั้ง</p></div>
          {targetStage==="LOST"&&<FormField label="Lost category" name="lostCategory" required><select className="control" name="lostCategory" defaultValue="" required><option value="" disabled>เลือกหมวดหมู่ Lost</option><option value="COMPETITOR">Competitor</option><option value="PRICE">Price</option><option value="NO_BUDGET">No Budget</option><option value="CUSTOMER_CANCELLED">Customer Cancelled</option><option value="TECHNICAL_LIMITATION">Technical Limitation</option><option value="OTHER">Other</option></select></FormField>}
          {targetStage==="CANCELLED"&&<div className="field full"><FormField label="เหตุผลที่ยกเลิก" name="cancelledReason" required><Textarea name="cancelledReason" required/></FormField></div>}
          <div className="field full"><FormField label={targetStage==="LOST"?"เหตุผลที่ Lost":"เหตุผล"} name="reason" required><Textarea name="reason" required/></FormField></div>
        </div>:<div className="notice warning">ไม่มีขั้นตอนปลายทางให้เลือก</div>}
      </div>
      <FormNotice state={state}/>
      <div className="actions"><button className="primary" disabled={pending||!targetStage}>{pending?"กำลังเปลี่ยน…":"ยืนยัน Transition"}</button></div>
    </div>
  </form>;
}
...
type EditableQuoteStatus = "DRAFT" | "APPROVED" | "REJECTED" | "RETURNED" | "SENT" | "ACCEPTED";
const EDITABLE_QUOTE_STATUSES: ReadonlyArray<{ value: EditableQuoteStatus; label: string }> = [
...
export function GovernedQuoteForm({ products, opportunities, quoteId, proposalId, initialOpportunityId, initialValidUntil, initialNotes, initialStatus, statusOverrideEnabled = false, initialLines }: { products: QuoteEditorProduct[]; opportunities: Array<{ id: string; name: string; customerName: string }>; quoteId?: string; proposalId?: string; initialOpportunityId?: string; initialValidUntil?: string; initialNotes?: string; initialStatus?: string; statusOverrideEnabled?: boolean; initialLines?: Array<Omit<QuoteEditorLine, "key">> }) {
  ...
  return <form action={action} className="quote-editor"><input type="hidden" name="idempotencyKey" value={key}/><input type="hidden" name="quoteId" value={quoteId ?? ""}/><input type="hidden" name="proposalId" value={proposalId ?? ""}/><input type="hidden" name="itemsJson" value={JSON.stringify(lines.map(({ productId, quantity, unitPrice, discountPct }) => ({ productId, quantity, unitPrice, discountPct })))}/>
    <section className="card quote-header-card"><div className="card-header"><div><span className="quote-step">01</span><div><strong>Quotation Header</strong><small>ข้อมูลหลักของใบเสนอราคาและ Opportunity</small></div></div><span className="badge muted">{quoteId ? "New Version" : "Draft Version"}</span></div><div className="card-body"><div className="form-grid">
      ...
      {quoteId&&<FormField label="Status" name="status" required help={statusOverrideEnabled?"เลือก Status ของ version ใหม่ได้โดยตรงระหว่างที่ Approval Control Center ปิดอยู่":"Status อยู่ภายใต้ Approval workflow"}><select id="status" className="control" name="status" defaultValue={EDITABLE_QUOTE_STATUSES.some((item)=>item.value===initialStatus)?initialStatus:"DRAFT"} disabled={!statusOverrideEnabled}>{EDITABLE_QUOTE_STATUSES.map((item)=><option key={item.value} value={item.value}>{item.label}</option>)}</select>{!statusOverrideEnabled&&<input type="hidden" name="status" value="DRAFT"/>}</FormField>}
    ...
    <FormNotice state={state}/><div className="quote-actions"><Link href="/quotes" className="secondary">ยกเลิก</Link><button className="primary" disabled={pending||lines.some((line)=>!line.productId)}>{pending?"กำลังสร้าง…":quoteId?"บันทึก Version ใหม่":"สร้าง Draft Version"}</button></div>
  </form>;
}
