...
export function GovernedQuoteForm({ products, opportunities, proposalId, initialOpportunityId }: { products: QuoteEditorProduct[]; opportunities: Array<{ id: string; name: string; customerName: string }>; proposalId?: string; initialOpportunityId?: string }) {
  ...
  return <form action={action} className="quote-editor"><input type="hidden" name="idempotencyKey" value={key}/><input type="hidden" name="proposalId" value={proposalId ?? ""}/><input type="hidden" name="itemsJson" value={JSON.stringify(lines.map(({ productId, quantity, unitPrice, discountPct }) => ({ productId, quantity, unitPrice, discountPct })))}/>
