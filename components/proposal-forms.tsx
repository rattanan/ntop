...
type CompareVersion={versionNumber:number;createdAt:string;sections:Array<{sectionCode:string;title:string;contentHtml:string}>};
export function ProposalVersionCompare({ versions }: { versions: CompareVersion[] }) {
  ...
  if(versions.length<2)return <section className="card"><div className="card-header"><strong>Compare Versions</strong></div><div className="card-body"><p className="help">ต้องมี Proposal อย่างน้อย 2 versions จึงจะเปรียบเทียบได้</p></div></section>;
  return <section className="card"><div className="card-header"><strong>Compare Versions</strong><div className="version-selectors"><select className="control" value={left} onChange={(e)=>setLeft(Number(e.target.value))}>{versions.map((v)=><option key={v.versionNumber} value={v.versionNumber}>v{v.versionNumber}</option>)}</select><span>vs</span><select className="control" value={right} onChange={(e)=>setRight(Number(e.target.value))}>{versions.map((v)=><option key={v.versionNumber} value={v.versionNumber}>v{v.versionNumber}</option>)}</select></div></div><div className="table-wrap"><table className="table proposal-compare"><thead><tr><th>Section</th><th>v{left}</th><th>v{right}</th></tr></thead><tbody>{codes.map((code)=>{const x=a?.sections.find((s)=>s.sectionCode===code);const y=b?.sections.find((s)=>s.sectionCode===code);const changed=x?.contentHtml!==y?.contentHtml;return <tr key={code} className={changed?"changed":""}><td>{x?.title??y?.title}</td><td>{x?.contentHtml?<div className="proposal-rich-content proposal-rich-content-compact" dangerouslySetInnerHTML={{__html:x.contentHtml}}/>:"—"}</td><td>{y?.contentHtml?<div className="proposal-rich-content proposal-rich-content-compact" dangerouslySetInnerHTML={{__html:y.contentHtml}}/>:"—"}</td></tr>})}</tbody></table></div></section>;
}
