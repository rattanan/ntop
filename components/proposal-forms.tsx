...
export function ProposalAiGenerator({ proposalId, version, products, disabled, availabilityMessage }: { proposalId: string; version: number; products: Array<{ id: string; code: string; name: string; category: string }>; disabled: boolean; availabilityMessage?: string }) {
  ...
  return <section className="card ai-proposal-card"><div className="card-header"><div><strong>AI Proposal Generator</strong><small>ใช้ Opportunity, Customer, Meeting Notes, Template และ Product ที่เลือก</small></div><span className="badge ai">AI Draft</span></div><div className="card-body"><input className="control" type="search" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="ค้นหารหัส ชื่อ หรือหมวด Product" aria-label="Search products" aria-controls="proposal-product-results" disabled={disabled}/><div className="proposal-product-picker" id="proposal-product-results" role="group" aria-label="Product search results">{visible.map((item)=><label key={item.id}><input type="checkbox" checked={selected.includes(item.id)} disabled={disabled} onChange={(event)=>setSelected((current)=>event.target.checked?[...current,item.id]:current.filter((id)=>id!==item.id))}/><span><strong>{item.code}</strong> {item.name}<small>{item.category}</small></span></label>)}{!visible.length&&<p className="proposal-product-empty" role="status">ไม่พบ Product ที่ค้นหา</p>}</div><p className="help">แสดงสูงสุด 50 รายการ · เลือก {selected.length} product · AI output ต้องตรวจและแก้ไขก่อนส่ง</p>{availabilityMessage&&<p className="notice" role="status">{availabilityMessage}</p>}{message&&<p className="help" role="status">{message}</p>}<button type="button" className="primary ai-action" onClick={generate} disabled={disabled||pending}>{pending?"Generating…":"✦ Generate Proposal"}</button></div></section>;
}
