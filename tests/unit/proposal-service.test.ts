...
function setup(approvalEnabled = true){
  ...
  return{repository,audit,service:new ProposalService(repository,audit,undefined,undefined,()=>approvalEnabled)};
}
...
describe("ProposalService",()=>{
  ...
  it("keeps Draft creation available while blocking approval submission when centrally disabled",async()=>{const{service,repository}=setup(false);await expect(service.create(actor,{opportunityId:"opp",name:"Enterprise Proposal",tags:[]},"corr","create-disabled")).resolves.toMatchObject({statusCode:"DRAFT"});await expect(service.transition(actor,"proposal",{expectedVersion:1,toStatusCode:"PENDING_REVIEW",comment:"Ready"},"corr","submit-disabled")).rejects.toBeInstanceOf(ApprovalWorkflowDisabledError);expect(repository.createVersion).not.toHaveBeenCalled();});
