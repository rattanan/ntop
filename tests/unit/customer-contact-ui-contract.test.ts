...
describe("Customer contact UI contract",()=>{
  ...
  it("separates Customer 360 into compact tabs with equally tall hierarchy and duplicate panels",()=>{for(const tab of["overview","contacts","governance","sales"]){expect(detail).toContain(`tabHref(\"${tab}\")`);}expect(detail).toContain('activeTab === "governance"');expect(detail).toContain("Duplicate candidates");expect(detail).toContain("<CustomerGovernanceActions");expect(css).toContain(".governance-summary { align-items:stretch; }");expect(css).toContain(".governance-summary>.card { height:100%;min-height:0;align-self:stretch; }");});
