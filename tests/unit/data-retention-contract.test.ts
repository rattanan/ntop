...
describe("Deletion and data-retention contract",()=>{
  ...
  it("provides governed UI actions without Lead, Opportunity, or Customer hard delete",()=>{const actions=read("components/data-retention-actions.tsx"),lead=read("components/lead-workflow-forms.tsx"),opportunity=read("components/workflow-forms.tsx"),detail=read("app/(portal)/opportunities/[id]/page.tsx"),migration=read("prisma/migrations/20260717090000_add_data_retention_policy/migration.sql");expect(actions).toContain("ลบแบบกู้คืนได้");expect(actions).toContain("กู้คืน");expect(lead).toContain("Archive Lead");expect(lead).toContain("Mark Invalid");expect(opportunity).toContain('targetStage==="CANCELLED"');expect(detail).toContain("STAGES.filter");expect(migration).toContain("'EXPIRE'");});
});
