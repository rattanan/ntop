...
const editPage=readFileSync(join(process.cwd(),"app/(portal)/leads/[id]/edit/page.tsx"),"utf8");
const workflowForms=readFileSync(join(process.cwd(),"components/lead-workflow-forms.tsx"),"utf8");
...
describe("Lead workflow foundation",()=>{
  ...
  it("links list rows to scoped detail and separates editing from conversion",()=>{expect(listPage).toContain("/leads/${lead.id}");expect(detailPage).toContain("buildLeadScopeWhere(context)");expect(detailPage).not.toContain("<LeadEditForm");expect(detailPage).toContain("<LeadConversionActions");expect(editPage).toContain("<LeadEditForm");expect(editPage).toContain("buildLeadScopeWhere(context)");});
  it("offers conversion for every active Lead status",()=>{expect(detailPage).not.toContain('lead.status==="QUALIFIED"&&canCoreUpdate');expect(workflowForms).not.toContain('pending||lead.status!=="QUALIFIED"');expect(workflowForms).not.toContain("Lead ต้องอยู่สถานะ");});
